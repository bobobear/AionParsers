namespace Jamie.ParserBase
{
	...
	public partial class CombineRecipe
	{
		...
		[XmlElement(Form = XmlSchemaForm.Unqualified)]
		public string qualification_race;

		[XmlIgnore]
		[DefaultValue(ItemRace.PC_ALL)]
		public ItemRace race {
			get {
				if (String.IsNullOrEmpty(qualification_race))
					return ItemRace.PC_ALL;
				string[] races = qualification_race.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
				if (races.Length > 1)
					return ItemRace.PC_ALL;

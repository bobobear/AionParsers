namespace Jamie.Items {
	...
	class Program {
		...
		static void Main(string[] args) {
			...
			//Console.WriteLine("Loading decomposable items...");
			//Utility.LoadDecomposableItemsFile(root);
			...
			foreach (var recipe in Utility.RecipeIndex.RecipeList) {
				if (recipe.race == ItemRace.PC_ALL)
					continue;
				...
				if (recipe.race == ItemRace.ASMODIANS)
		...
		private static void ExportRecipes(XmlWriterSettings saveSettings, string outputPath) {
			...
			foreach (var recipe in Utility.RecipeIndex.RecipeList) {
				...
				template.name = Regex.Replace(Utility.StringIndex.GetString(recipe.desc), @"[^\u0030-\u007A\u0020]", string.Empty);

				// zp - return substring of parent item for matching test items 
				string recipeName = "STR_" + recipe.name.ToUpper();
				if(recipeName.Substring(recipeName.Length - 5, 5) == "_TEST")
					recipeName = recipeName.Substring(0, recipeName.Length - 5);

				int stringIndex = Utility.StringIndex[recipeName];

				descriptionID = stringIndex * 2 + 1;

				// zp - hard code 1 name ID, need to investigate why string is not in strings
				if (template.id == 155100341) descriptionID = 1590723;

				if (descriptionID < 1) {
					Console.Write("Pausing (Press Enter): Bad DescriptionID!" + descriptionID + " recipeID: " + template.id);
					Console.ReadLine(); //Pause	
				}

				template.nameid = descriptionID;

				template.skillid = Utility.GetSkillIdFromName(recipe.combineskill.ToString());
				if (template.skillid < 1) {
					Console.Write("Pausing (Press Enter): Unknown recipe SkillID - recipeId: " + recipe.id);
					Console.ReadLine(); //Pause	
				}

				//template.race = (skillRace)Enum.Parse(typeof(skillRace), recipe.qualification_race.ToString().ToUpper());
				// zp
				template.race = recipe.race;
				...
				template.max_production_count = recipe.max_production_count;
				template.craft_delay_id = recipe.craft_delay_id;
				template.craft_delay_time = recipe.craft_delay_time;
...
*/
		...
		public static string GetItemCategory(string iconName) {
			...
			if (iconName.StartsWith("icon_cash_item_")) {
				skip = 3;
			}
			...
			string final = sb.ToString().TrimEnd('_').ToUpper();

			// zp
			string[] split = final.Split('_');
			if (split.Length > 1) {
				switch (split[1]) {
					case "BODY": return "JACKET";
					case "TORSO": return "JACKET";
			...
			switch (final) {
				...
				case "H2SWORD": return "GREATSWORD";
				case "2HSWORD": return "GREATSWORD";
				case "HELM": return "HELMET";
				case "CAP": return "HEAD";
				case "GLASSES": return "HEAD";
			}
		...
		private static void ExportItemGroups(XmlWriterSettings saveSettings, string outputPath) {
			...
			foreach (var item in Utility.ItemIndex.ItemList) {
				...
				//exportItem.weaponStats = item.GetWeaponStats();
				...
				if (exportItem.raceInternal != ItemRace.PC_ALL) {
					// don't change
				}
				else {
				...
				if (nameParts.Length > 2) {
					...
					if ((exportItem.itemGroup == "r" || exportItem.itemGroup == "R") &&
						...
						if (exportItem.originalItem.level >= 52)
							exportItem.raceInternal = ItemRace.PC_ALL;
            ...
            {
                { "Soul", ItemRace.PC_ALL }, { "Sky", ItemRace.PC_ALL }, { "Eldritch", ItemRace.PC_ALL },
                { "Grounded", ItemRace.PC_ALL }, { "Angry", ItemRace.PC_ALL }, { "Battlefield", ItemRace.PC_ALL },
                { "Zealous", ItemRace.PC_ALL }, { "Wraith's", ItemRace.PC_ALL }, { "Fascination", ItemRace.PC_ALL },
                { "Eternity", ItemRace.PC_ALL }, { "Blood Seeking", ItemRace.PC_ALL }, { "Divisive", ItemRace.PC_ALL },
                { "Celestial", ItemRace.PC_ALL }, { "Everwatcher's", ItemRace.PC_ALL }, { "Old Guard's", ItemRace.PC_ALL },
                { "Invader's", ItemRace.PC_ALL }, { "Ancient Daeva's", ItemRace.PC_ALL }, { "Carved", ItemRace.PC_ALL },
                { "Anonymous", ItemRace.PC_ALL }, { "Reddened", ItemRace.PC_ALL }, { "White", ItemRace.PC_ALL },
                { "Red Dragon Legion", ItemRace.PC_ALL }, { "Rain", ItemRace.PC_ALL }, { "Immemorial", ItemRace.PC_ALL },
                { "Departed Soul's", ItemRace.PC_ALL }, { "Purehorn's", ItemRace.PC_ALL }, { "Calamity", ItemRace.PC_ALL },
                { "Sleeper's", ItemRace.PC_ALL }, { "Buried", ItemRace.PC_ALL }, { "Immortal", ItemRace.PC_ALL },
                { "Fallen Legionary's", ItemRace.PC_ALL }, { "Nebulous", ItemRace.PC_ALL }, { "Reaper's", ItemRace.PC_ALL },
                { "Odious", ItemRace.PC_ALL }, { "Protector's", ItemRace.PC_ALL }, { "Martyr's", ItemRace.PC_ALL },
                { "Oblate", ItemRace.PC_ALL }, { "Dimensional", ItemRace.PC_ALL }, { "Giant Lord's", ItemRace.PC_ALL },
                { "Ancient Giant's", ItemRace.PC_ALL }, { "Restoration", ItemRace.PC_ALL }, { "Miracle", ItemRace.PC_ALL },
                { "Comet", ItemRace.PC_ALL }, { "Meteor", ItemRace.PC_ALL }, { "Ancient Hero's", ItemRace.PC_ALL },
                { "Giant's", ItemRace.PC_ALL }, { "Elder", ItemRace.PC_ALL }, { "Catastrophic", ItemRace.PC_ALL },
                { "Horrific", ItemRace.PC_ALL }, { "Chaos", ItemRace.PC_ALL }, { "Dolan", ItemRace.PC_ALL }, { "Judgment", ItemRace.PC_ALL }, 
                { "Demolition", ItemRace.PC_ALL }, { "Gracefull", ItemRace.PC_ALL }, { "Death", ItemRace.PC_ALL },
                { "Life", ItemRace.PC_ALL }, { "Destructive", ItemRace.PC_ALL }, { "Storm", ItemRace.PC_ALL }, { "Sage", ItemRace.PC_ALL },
                { "Sunset", ItemRace.PC_ALL }, { "Angel's", ItemRace.PC_ALL }, { "Guarded", ItemRace.PC_ALL }, { "Dispelling", ItemRace.PC_ALL },
                { "Archbishop", ItemRace.PC_ALL }, { "Restless", ItemRace.PC_ALL }, { "Prayer", ItemRace.PC_ALL }, { "Torment", ItemRace.PC_ALL }
            };
